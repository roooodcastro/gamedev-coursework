...
Face::Face(const Face &copy) {
	this->vertexIndexes = new int[copy.quad ? 4 : 3];
	this->texCoords = new int[copy.quad ? 4 : 3];
	this->normals = new int[copy.quad ? 4 : 3];
		...
		for (unsigned i = 0; i < faces->size(); i++) {
			...
			if (face->material->getTexture() && face->material->getTexture()->isTextureValid()) {
				if (face->material->getTexture()->getTextureId() != currentTexId) {
					...
					if (currentTexId >= 0) {
						glDrawArrays(face->quad ? GL_QUADS : GL_TRIANGLES, vertexOffset, vertexCount);
